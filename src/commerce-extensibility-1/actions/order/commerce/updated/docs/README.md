...
Any values from the environment could be accessed from `params`. Pass the required parameters by the action by configuring them in the `src/commerce-extensibility-1/actions/order/commerce/actions.config.yaml` under `uodated -> inputs` as follows:
