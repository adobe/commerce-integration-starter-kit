# Integrate a third party product created event with Adobe Commerce.
...
This function delegates to the `createProduct` method in the `src/commerce-extensibility-1/actions/product/commerce-product-api-client.js` the interaction with the Commerce API.
Any parameters needed from the execution environment could be accessed from `params`.
These parameters can be passed on the action by configuring them in the `src/commerce-extensibility-1/actions/product/external/actions.config.yaml` under `created -> inputs` as follows:
