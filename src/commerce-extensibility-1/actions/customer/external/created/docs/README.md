# Integrate a third party customer created event with Adobe Commerce.
...
![Bidirectional customer synchronization highlighting customer creation from the backoffice](ExternalCustomerCreateSync.png "Backoffice customer creation synchronization")
...
This function delegates to the `createCustomer` method in the `src/commerce-extensibility-1/actions/customer/commerce-customer-api-client.js` the interaction with the Commerce API.
Any parameters needed from the execution environment could be accessed from `params`.
These parameters can be passed on the action by configuring them in the `src/commerce-extensibility-1/actions/customer/external/actions.config.yaml` under `created -> inputs` as follows:
