...
![Bidirectional customer group synchronization highlighting deletion from Adobe Commerce](CommerceCustomerGroupDeleteSync.png "Commerce customer group deletion synchronization")
...
Any values from the environment could be accessed from `params`. Pass the required parameters by the action by configuring them in the `src/commerce-extensibility-1/actions/customer/commerce/actions.config.yaml` under `group-deleted -> inputs` as follows:
