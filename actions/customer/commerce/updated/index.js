...
/**
 * This action is on charge of sending updated customer information in Adobe commerce to external back-office application
 *
...
async function main (params) {
  ...
  try {
    ...
    if (!validation.success) {
      logger.error(`[Customer][Commerce][Updated] Validation failed with error: ${validation.message}`)
    ...
    logger.debug(`[Customer][Commerce][Updated] Preprocess data: ${JSON.stringify(params)}`)
    const preProcessed = preProcess(params, transformedData)

    logger.debug(`[Customer][Commerce][Updated] Start sending data: ${JSON.stringify(params)}`)
    const result = await sendData(params, transformedData, preProcessed)
    if (!result.success) {
      logger.error(`[Customer][Commerce][Updated] ${result.message}`)
      return actionErrorResponse(result.statusCode, result.message)
    }

    logger.debug(`[Customer][Commerce][Updated] Postprocess data: ${JSON.stringify(params)}`)
