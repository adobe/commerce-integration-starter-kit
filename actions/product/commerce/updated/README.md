...
Any need for parameters from environment could be access from `params`. Add the needed parameter in the `actions/product/product.actions.config.yaml` under `commerceupdated -> inputs` as follows:
```yaml
commerceupdated:
