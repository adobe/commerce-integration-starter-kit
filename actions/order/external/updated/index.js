...
async function main (params) {
  ...
  logger.info('[OrderStatus][External][Updated] Start processing request')
  logger.debug(`[OrderStatus][External][Updated] Action main params: ${stringParameters(params)}`)

  try {
    logger.debug(`[OrderStatus][External][Updated] Validate data: ${JSON.stringify(params.data)}`)
    const validation = validateData(params)
    if (!validation.success) {
      logger.error(`[OrderStatus][External][Updated] Validation failed with error: ${validation.message}`)
      return actionErrorResponse(HTTP_BAD_REQUEST, validation.message)
    }

    logger.debug(`[OrderStatus][External][Updated] Transform data: ${JSON.stringify(params)}`)
    const transformed = transformData(params)

    logger.debug(`[OrderStatus][External][Updated] Preprocess data: ${JSON.stringify(params)}`)
    ...
    logger.debug('[OrderStatus][External][Updated] Process finished successfully')
