...
This function delegates to the `createCustomerGroup` method in the `actions/customer-group/commerceCustomerGroupApiClient.js` the interaction with the Commerce API.
Any parameters needed from the execution environment could be access from `params`. 
These parameters can be passed on the action by configuring them in the  `actions/customer-group/external/actions.config.yaml` under `created -> inputs` as follows:
```yaml
created:
